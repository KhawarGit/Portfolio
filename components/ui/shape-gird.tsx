...
const hoverTimeoutMs = 500; // adjust as needed
...
// remove expired trail entries
trailCells.current = trailCells.current.filter(
  (t) => now - t.t < TRAIL_LIFETIME
);
  // 1. If idle → kill hover completely
  ...
  // 2. Trail logic (ALWAYS runs but decays faster when idle)
...
const base = Math.pow(1 - tNorm, 2.2); // smoother luxury falloff
  ...
  // 3. CRITICAL FIX: force fade everything when idle
  if (isIdle) {
  ...
  // 4. Ensure all target keys exist
  ...
  for (const [key, opacity] of cellOpacities.current) {
    ...
    // const next = opacity + (target - opacity) * 0.12;
...
const next = opacity + (target - opacity) * easing;
    ...
    // }
    if (next < 0.01) {
  cellOpacities.current.set(key, 0); // keep it alive but invisible
} else {
  cellOpacities.current.set(key, next);
}
  ...
  // ONLY clear when everything is basically invisible
  if (!stillAlive) {
